import { ChevronDown, ChevronUp, Ellipsis, LogOut, Trash, X } from "lucide-react";
...
const ChatHeader: React.FC<ChatHeaderProps> = ({ onOpenModal, onSearchChat, onClearSearch, onGoToChat }) => {
  const { selectedUser, setSelectedUser, filteredMessages, messages } = useChatStore();
          ...
          <div className="hidden sm:flex items-center gap-2 h-10 absolute left-1/2 transform -translate-x-1/2">
            ...
            {filteredMessages.length > 0 && searchQuery && (
              <div className=" transform flex ">
                <button onClick={handleSearchUp} className="btn btn-circle btn-xs btn-ghost hover:bg-base-300" title="Search up" disabled={currentIndex === 0}>
                  <ChevronUp size={16} />
                </button>
